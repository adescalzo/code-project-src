...
source: https://fast-endpoints.com/docs/swagger-support#enabling-xml-documentation
date_published: unknown
date_captured: 2025-08-27T15:45:34.033Z
domain: fast-endpoints.com
author: Unknown
category: general
technologies: [FastEndpoints, NSwag, FastEndpoints.Swagger, SwaggerUI, Scalar, ASP.NET Core, .NET, System.Text.Json, Newtonsoft.Json, Kiota, OpenAPI, JWT Bearer Auth, MSBuild, Output Caching Middleware, LinkGenerator, OpenApiSecuritySchemeType, OpenApiSecurityApiKeyLocation, JwtBearerDefaults.AuthenticationScheme, NJsonSchema, PrimitiveTypeMapper]
programming_languages: [C#, TypeScript]
tags: [swagger, openapi, api-documentation, fastendpoints, dotnet, web-api, client-generation, nswag, api-security, serialization]
key_concepts: [API documentation, OpenAPI specification, client generation, model-binding, authentication-schemes, serialization, dependency-injection, middleware-configuration]
code_examples: false
difficulty_level: intermediate
summary: |
  [This article details the comprehensive Swagger/OpenAPI support within the FastEndpoints framework, leveraging the NSwag library. It covers initial setup, configuration of Swagger documents, and automatic inference of API metadata like `Accepts` and `Produces` based on endpoint DTOs and validators. The guide explains how to customize endpoint descriptions, request parameters, and response examples using both code and XML comments. Furthermore, it delves into advanced topics such as multiple authentication schemes, custom serialization settings, and client generation using Microsoft's Kiota library, including options for download endpoints and MSBuild integration. The content also provides guidance on conditional middleware configuration for different application modes, ensuring flexible and robust API documentation and client tooling.]
---
```

# Swagger Support | FastEndpoints
...
Client generation is facilitated by the excellent [Kiota](https://learn.microsoft.com/en-us/openapi/kiota/design) library by Microsoft. You can use our wrapper library to integrate client generation straight into your FastEndpoints app instead of using their CLI tools. Clients can be easily generated for [supported languages](https://learn.microsoft.com/en-us/openapi/kiota/using#accepted-values-7) in the following ways:
